...
export async function getDashboard(prisma:PrismaClient,userId:string,portal:AnalyticsPortal,preset:TimePreset="FINANCIAL_YEAR"){
  ...
  const lines=await prisma.seeraOrderLine.findMany({where:{order:orderWhere},include:{order:{select:{id:true,status:true,createdAt:true,salespersonId:true,buyerPartnerId:true}}}});
  const visits=await prisma.seeraVisit.findMany({where:{checkedInAt:{gte:range.from,lte:range.to},...(scope.employeeIds?{workSession:{employeeId:{in:scope.employeeIds}}}:{})},select:{id:true,outcome:true,retailerId:true}});
  const movements=await prisma.seeraInventoryMovement.findMany({where:{occurredAt:{gte:range.from,lte:range.to},...(scope.partyIds?{partyId:{in:scope.partyIds}}:{})},select:{quantity:true,direction:true,type:true,occurredAt:true}});
  const targets=await prisma.seeraTarget.findMany({where:{periodEnd:{gte:range.from},periodStart:{lte:range.to},...(scope.employeeIds?{employeeId:{in:scope.employeeIds}}:{})}});
  const collections=await prisma.seeraCollectionEntry.aggregate({where:{collectedAt:{gte:range.from,lte:range.to},...(scope.employeeIds?{actorId:{in:scope.employeeIds}}:{})},_sum:{amount:true}});
  const partners=await prisma.seeraPartner.count({where:{lifecycle:"ACTIVE",...(portal==="distributor"?{type:"DISTRIBUTOR"}:portal==="super-stockist"?{type:"SUPER_STOCKIST"}:{})}});
  const paymentProofs=await prisma.seeraPaymentProof.count({where:{status:"SUBMITTED"}});
  const reconciliations=await prisma.seeraFinancialReconciliation.count({where:{status:"OPEN"}});
