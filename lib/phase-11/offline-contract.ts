import {z} from "zod";
// ADD_CUSTOMER_DRAFT (production stability pass): submitAddCustomer previously called send()
// directly, bypassing runOrQueue entirely - Add Customer was the one major field mutation with NO
// offline-queue fallback on a genuine network failure, unlike every sibling action (Order,
// Checkout, End Day). VISIT_DRAFT alone can't cover this - it requires an already-persisted
// retailerId (check-in to an EXISTING retailer); Add Customer's whole point is creating a NEW one.
export const OFFLINE_ACTIONS=["VISIT_DRAFT","VISIT_CHECK_OUT","ORDER_DRAFT","NO_ORDER_DRAFT","COLLECTION_DRAFT","FOLLOW_UP_DRAFT","MARKET_INTELLIGENCE_DRAFT","DISTRIBUTOR_PROSPECT_DRAFT","DAY_END_DRAFT","DISTRIBUTOR_DECISION_DRAFT","DISTRIBUTOR_DELIVERY_DRAFT","DISTRIBUTOR_REMAINING_DRAFT","DISTRIBUTOR_CLOSE_REMAINING_DRAFT","ADD_CUSTOMER_DRAFT"] as const;
