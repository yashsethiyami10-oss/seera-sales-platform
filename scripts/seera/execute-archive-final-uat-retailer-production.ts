...
async function main() {
  ...
  const res = await fetch(`${BASE}/api/distribution/operations`, {
    ...
    body: JSON.stringify({
      action: "archive-retailer",
      payload: { retailerId: "cmt4ulzxn0041tkqugf1vbfy9", reason: "Final closure (23-Aug): sanctioned temporary FINAL-UAT-RETAILER ('MUV CARE CO.') never archived after production verification — archiving per governed cleanup rule (has order/visit dependencies, archive not hard-delete)." },
    }),
  });
  console.log("archive status:", res.status, await res.json().catch(() => ({})));
