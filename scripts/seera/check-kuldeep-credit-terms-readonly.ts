...
async function main() {
  ...
  const terms = await db.seeraCreditTerm.findMany({ where: { distributorId: "cmsvy1mj0004s1154q0fc8urs" } });
  console.log(`Kuldeep credit terms rows: ${terms.length}`, JSON.stringify(terms));
}
