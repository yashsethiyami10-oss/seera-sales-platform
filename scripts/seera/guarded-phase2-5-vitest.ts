...
const target=authorizeDatabaseCommand({intendedRole:"test",write:true,targetUrl:test,productionUrl:production,testUrl:test});const runtime=new URL(test);runtime.searchParams.set("connection_limit","4");runtime.searchParams.set("pool_timeout","60");runtime.searchParams.set("statement_timeout","60000");runtime.searchParams.set("connect_timeout","20");
console.log(`[SEERA DB GUARD] role=${target.role} host=${target.host} database=${target.database} fingerprint=${target.fingerprint}`);
const cli=path.join(root,"node_modules","vitest","vitest.mjs");const result=spawnSync(process.execPath,[cli,"run","--config","vitest.phase2-5.integration.config.ts",...process.argv.slice(2)],{cwd:root,env:{...process.env,DATABASE_URL:runtime.toString(),TEST_DATABASE_URL:test,SEERA_DATABASE_ROLE:"test"},stdio:"inherit"});if(result.error)throw result.error;
