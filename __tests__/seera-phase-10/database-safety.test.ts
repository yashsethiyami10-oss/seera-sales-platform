...
describe("fail-closed DB test runtime",()=>{
 ...
 it("rejects unknown runtime target",()=>expect(()=>authorizeTestRuntime({runtimeRole:"test",runtimeDatabaseUrl:"postgresql://u:p@other.example/x",configuredProductionUrl:prod,configuredTestUrl:test})).toThrow(/does not match the configured Seera TEST project\/database/));
