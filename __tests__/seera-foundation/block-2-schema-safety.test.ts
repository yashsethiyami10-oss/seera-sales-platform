...
describe("Block 2 schema and RBAC boundary", () => {
  ...
  it("defines all canonical Phase 1 roles and namespaced permissions", () => {
    const requiredFoundationRoles = ["FOUNDER_SUPER_ADMIN", "COMPANY_ADMIN", "ACCOUNTS_MANAGER", "SALES_MANAGER", "SALES_EXECUTIVE", "DISTRIBUTOR_OWNER", "SUPER_STOCKIST_OWNER", "RETAILER_USER", "READ_ONLY_AUDITOR"];
    expect(PHASE_1_ROLE_CODES).toEqual(expect.arrayContaining(requiredFoundationRoles));
    expect(new Set(PHASE_1_ROLE_CODES).size).toBe(PHASE_1_ROLE_CODES.length);
