...
import { AppUpdateCheck } from "@/lib/seera/app-update-check";
...
export function AppShell({ children, nav, language, user, role, portal, environment }: { children: React.ReactNode; nav: ShellNavItem[]; language: Phase1Language; user: { name: string; email: string }; role: string; portal: string; environment: string }) {
  ...
  return <div className={styles.shell}><NativeShellBridge /><AppUpdateCheck language={language} /><button className={styles.mobileBackdrop} data-open={open} aria-label={t.close} onClick={() => setOpen(false)} /><aside className={styles.sidebar} data-open={open} aria-label="Primary navigation"><div className={styles.brand}><Image className={styles.brandLogo} src="/seera logo.png" alt="Seera" width={2560} height={1990} priority /><Image className={styles.brandTagline} src="/seera tagline.png" alt="Sarv Shaktiman" width={2172} height={724} priority /></div><nav className={styles.nav}>{sections.map((section, index) => {
  ...
  })}</nav><div className={styles.sidebarUser}><div className={styles.sidebarUserMeta}><span className={styles.sidebarUserName}>{user.name}</span><span className={styles.sidebarUserRole}>{role}</span></div><button type="button" className={styles.sidebarLogout} disabled={busy} onClick={logout}><span aria-hidden>⎋</span>{t.logout}</button></div><div className={styles.footer}>{t.workspace}<br />{environment}</div></aside><div className={styles.main}><header className={styles.header}><div className={styles.headerLeft}><button className={styles.menu} aria-label={t.menu} onClick={() => setOpen(true)}>☰</button><Image className={styles.mobileBrand} src="/seera logo.png" alt="Seera" width={2560} height={1990} priority /><span className={styles.crumb}>Seera / {pathname.split("/").filter(Boolean).slice(2).join(" / ") || portal}</span><span className={styles.roleBadge}>{role}</span></div><div className={styles.headerActions}><Link href={`/portal/${portal}/notifications`} aria-label={t.notifications}>◉ {t.notifications}</Link><button disabled={busy} onClick={() => changeLanguage(language === "EN" ? "HI" : "EN")}>{language === "EN" ? "हिन्दी" : "English"}</button><details className={styles.user}><summary aria-label={`${user.name} — ${t.profile} / ${t.logout}`}><span>{user.name}</span>●</summary><div className={styles.userMenu}><Link href={`/portal/${portal}/profile`}>{t.profile}</Link><button disabled={busy} onClick={logout}>{t.logout}</button></div></details></div></header><main className={styles.content}>{children}</main></div></div>;
}
