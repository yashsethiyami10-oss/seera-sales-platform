...
import { createDistributorForSuperStockist, updateDistributorCreditPolicy, createSuperStockist, createCompanyDirectPartner, reassignDistributorToSuperStockist, settleDistributorClosureStock, setCompanyDirectEligibility, setPartnerBillingProfile, createStandaloneDistributor, updatePartnerTerritories } from "@/lib/sales-distribution/distributor-management-service";
...
export async function POST(request: Request) {
  try {
    ...
    else if(action==="update-partner-territories") result=await updatePartnerTerritories(prisma,user.id,z.object({partnerId:z.string(),territoryIds:z.array(z.string()),reason:z.string().min(3)}).parse(payload));
