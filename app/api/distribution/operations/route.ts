...
import { createDistributorForSuperStockist, updateDistributorCreditPolicy, createSuperStockist, reassignDistributorToSuperStockist, settleDistributorClosureStock } from "@/lib/sales-distribution/distributor-management-service";
...
export async function POST(request: Request) {
  try {
    ...
    else if(action==="update-billing-draft") result=await updateBillingDraft(prisma,user.id,z.string().parse((payload as {documentId:unknown}).documentId),z.object({paymentTerms:z.string().optional(),notes:z.string().optional(),lines:z.array(z.object({skuId:z.string(),quantity:z.number().positive(),rate:z.number().nonnegative(),discountPct:z.number().min(0).max(100).optional(),taxRate:z.number().min(0).max(100).optional()})).min(1)}).parse(payload));
