...
import { uploadManufacturingDocument } from "@/lib/manufacturing/document-service";

const metadata = z.object({
  entityType: z.enum(["SeeraSop", "SeeraGrn", "SeeraBatchQc", "SeeraProductionBatch", "SeeraDeviationRecord", "SeeraManufacturingMaterial", "SeeraStockCount", "SeeraBom"]),
...
export async function POST(request: Request) {
  try {
    ...
    enforceRateLimit(`manufacturing-documents-upload:${user.id}`, 30, 60_000);
    ...
    const result = await uploadManufacturingDocument(prisma, user.id, { ...data, originalName: file.name, mimeType: file.type, bytes: new Uint8Array(await file.arrayBuffer()) });
